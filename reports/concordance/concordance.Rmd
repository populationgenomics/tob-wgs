...
params:
  ...
  WGS_MT: "/Users/peterd/projects/tob-wgs/nogit/data/bucket/cpg-tob-wgs-test/raw/test-v1-raw.mt"
...
use_condaenv("concordance", required = TRUE)
```
...
# export concordance stats per sample
conc_per_sample = cols_conc.to_pandas()
```

## Results

```{r concordance_results}
get_conc_stats <- function(l) {
  list_to_matrix <- function(l) {
    conc_names <- c("missing_variant", "missing_gt", "homref", "het", "homalt")
    m <- l %>%
      set_names(glue("{conc_names}_left")) %>%
      bind_rows() %>%
      as.matrix() %>%
      t()
  ...
  l %>% list_to_matrix() %>% mat_conc_stats()
}

res <- py$conc_per_sample %>%
  mutate(conc2 = map(concordance, ~get_conc_stats(.x))) %>%
  unnest(conc2) %>%
