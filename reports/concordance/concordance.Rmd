...
3. For a sample with a given TOBID of TOBxxxx, grab the `CHR_POS`, `REF`, `ALT`,
   and `SNPID-sample` columns (based on SNPID <-> TOBID mapping file) from the
   SNPchip VCF. Do the same for the corresponding variant site-only WGS VCF.
   Also discard variants that are homozygous reference in the SNPchip VCF
   for TOBxxxx. The WGS VCF will have a lot more variants than the SNPchip VCF
   (~5 million vs. ~0.15 million).
4. Do a left join of SNPchip with WGS i.e. keep all SNPchip rows and grab the
   WGS genotypes + `REF`/`ALT` alleles corresponding to the SNPchip `CHR_POS`
   coordinates - these values will be `NA` for cases where there is no
   corresponding WGS variant. More than 99% of the SNPchip sites should
   correspond to a WGS call.
5. Calculate fraction of genotypes that match between WGS and SNPchip sites
   (i.e. `n_gt_match / n_snp`).
...
```{r funcs}
# Given two dfs with chr_pos, ref, alt, and gt columns, calculate concordance
# stats between the two.
concordance_stats <- function(wgs, snp) {
  req_cols <- c("chrpos", "ref", "alt", "gt")
  assert_that(all(req_cols %in% colnames(snp)),
              all(req_cols %in% colnames(wgs)))
  gt_match <- snp %>%
    dplyr::left_join(wgs, by = "chrpos", suffix = c(".snp", ".wgs")) %>%
    # ignore phasing
    dplyr::mutate(gt.wgs = sub("\\|", "/", gt.wgs),
                  gt_equal = gt.snp == gt.wgs)
  ...
  n_gt_match <- gt_match %>%
    dplyr::filter(gt_equal == TRUE) %>%
    nrow()
  n_gt_nomatch <- gt_match %>%
    dplyr::filter(gt_equal %in% c(FALSE, NA)) %>%
    nrow()
...
# Run dockerised gvcftools to extract variant sites from the input GVCF file
run_gvcftools <- function(input, output) {
  assert_that(
    length(input) == 1, length(output) == 1,
    file.exists(input),  grepl("vcf\\.gz", input)
  )
  ...
  gvcftools_cmd <-glue::glue("'gzip -dc /data/input/{ibname} | ",
                        "extract_variants | gzip -c > /data/output/{obname}'")

  system(glue::glue("{docker_cmd} {gvcftools_cmd}"))
}

# Grab sample column with given tob_id from SNPchip df.
# Filter out homozygous reference genotypes.
select_snpchip_sample <- function(tob_id, snpchip, id_map) {
  assert_that(is.data.frame(snpchip),
              all(c("chrpos", "ref", "alt") %in% colnames(snpchip)))
  chipid <- tobid2chipid(tob_id, id_map)
  snpchip %>%
    dplyr::select(chrpos, ref, alt, gt = dplyr::all_of(chipid)) %>%
    dplyr::filter(!gt %in% "0/0") # homref
}

# Read WGS VCF (containing extracted variant sites).
# The tob_id is used to verify this is the sample we want.
read_wgs_vcf <- function(wgsvcf, tob_id) {
  dt <- data.table::fread(
    cmd = glue::glue("zgrep -v '^##' {wgsvcf}"), sep = "\t", na.strings = ".",
    drop = c("QUAL", "ID", "FILTER", "INFO", "FORMAT"), data.table = FALSE
  )
  assert_that(length(tob_id) == 1, tob_id %in% colnames(dt))

  dt %>%
    dplyr::rename(chr = `#CHROM`, pos = POS, ref = REF, alt = ALT) %>%
    dplyr::mutate(
      gt = sub("(^.*?):.*", "\\1", .data[[tob_id]]),
      chrpos = glue::glue("{chr}_{pos}")) %>%
    dplyr::select(chrpos, ref, alt, gt) %>%
    tibble::as_tibble()
}


# Given an id_map df with sample_id and tob_id, and a TOB ID,
# output the corresponding SNPchip VCF sample_id.
tobid2chipid <- function(tobid, idmap) {
  assert_that(
    is.character(tobid),
    length(tobid) == 1,
    is.data.frame(idmap),
    all(c("sample_id", "tob_id") %in% names(idmap))
  )
  chipid <- idmap %>%
    dplyr::filter(tob_id == tobid)
  if (nrow(chipid) == 1) {
    return(chipid$sample_id)
...
# Subset SNPchip df by chromosome and count
...
count_gt_per_chrom <- function(d, chrom) {
  ...
  assert_that(is.data.frame(d), "chr" %in% colnames(d),
              length(chrom) == 1, chrom %in% 1:22)
  d %>%
    dplyr::filter(chr %in% chrom) %>%
    tidyr::pivot_longer(-chr, names_to = "sample_id", values_to = "gt") %>%
    dplyr::group_by(chr, sample_id) %>%
    dplyr::count(gt)
}
...
```{r vars}
chip_vcf_raw <- here("nogit/data/snpchip/raw/onek1k_pre_imputation_genotypes_2021-Mar-17.vcf.bgz")
chip_ids_file <- here("nogit/data/snpchip/raw/OneK1K_sample_IDs_2021-Apr-15.xlsx")
```

```{r snpchip_stats}
BCFTOOLS <- glue::glue("~/conda/envs/bioinf/bin/bcftools")

# grab the sample names from the SNPchip VCF - 1,034 samples
chip_sample_nms <-
  system(glue::glue("{BCFTOOLS} query -l {chip_vcf_raw}"), intern = TRUE) %>%
  dplyr::as_tibble() %>%
...
chip_varcount <-
  system(glue::glue("{BCFTOOLS} stats {chip_vcf_raw} | ",
              "grep -v '^#' | ",
...
chip_id2tob_raw <-
  ...
  janitor::clean_names() %>%
  mutate(x1 = person == glue::glue("{index_id_fid}_{internal_id_iid}"),
         tob_id2 = formatC(tob_id, width = 5, format = "d", flag = "0"),
         tob_id2 = glue::glue("TOB-{tob_id2}"),
         x2 = tob_id2 == tob_dark_id,
         tob_id_final = glue::glue("TOB{tob_id}"))
...
- Apart from 51 SNPchip VCF samples that do not have a TOB ID, let's just get a
  clean map:
...
readr::write_tsv(
  x = chip_id2tob_clean,
  file = here("nogit/data/snpchip/processed",
              "sample2tobid_2021-Apr-20.tsv"))

# export samples that have TOB IDs, for use with bcftools reheader
chip_id2tob_clean %>%
  dplyr::filter(!is.na(tob_id)) %>%
  dplyr::select(sample_id, tob_id) %>%
  readr::write_tsv(
    file = here("nogit/data/snpchip/processed",
                "sample2tobid_bcftools_reheader_2021-May-17.tsv"),
    col_names = FALSE)
```
...
chip_geno <-
  data.table::fread(
    cmd = glue::glue("zgrep -v '^##' {chip_vcf_raw}"),
    sep = "\t", na.strings = ".",
...
```{r plot_geno1, fig.width=12, fig.height=20, eval=FALSE}
...
counts_gt1 <-
  ...
  dplyr::left_join(chip_id2tob_clean, by = c("sample_id"))
...
p1 <- counts_gt1 %>%
  ...
  ggtitle(glue::glue("SNPchip GT counts per chromosome for {nrow(count(counts_gt1, sample_id))} samples."))
...
```{r plot_geno2, fig.width=10, fig.height=12, eval=FALSE}
# jitter object for proper labels
jitter_pos1 <- ggplot2::position_jitter(width = 0.20, height = 0, seed = 123)

# violin per gt
p2 <- counts_gt1 %>%
  dplyr::group_by(gt, sample_id) %>%
  dplyr::summarise(count = sum(count), .groups = "drop_last") %>%
  dplyr::left_join(chip_id2tob_clean, by = "sample_id") %>%
  dplyr::mutate(
    # too many, so use a large multiplication factor
    outlier = dplyr::if_else(is_outlier(count, 6),
                             glue::glue("{tob_id} ({count})"), NA_character_)) %>%
  ggplot(aes(x = "", y = count)) +
  geom_violin() +
  ...
  ggtitle(glue::glue("SNPchip GT counts for {nrow(count(counts_gt1, sample_id))} samples."))
...
run_gvcftools(input = glue::glue("{idir}/TOB1524.g.vcf.gz"),
              output = glue::glue("{odir}/TOB1524_extracted_sites.vcf.gz"))
run_gvcftools(input = glue::glue("{idir}/TOB1532.g.vcf.gz"),
              output = glue::glue("{odir}/TOB1532_extracted_sites.vcf.gz"))
run_gvcftools(input = glue::glue("{idir}/TOB1533.g.vcf.gz"),
              output = glue::glue("{odir}/TOB1533_extracted_sites.vcf.gz"))
```
...
chip_geno38 <-
  data.table::fread(
    cmd = glue::glue("zgrep -v '^##' {chip_vcf_raw_38}"),
    sep = "\t", na.strings = ".",
    drop = c("QUAL", "FILTER", "INFO", "FORMAT"),
    data.table = FALSE) %>%
  dplyr::as_tibble() %>%
  dplyr::rename(chr = `#CHROM`, pos = POS, id = ID, ref = REF, alt = ALT) %>%
  mutate(chrpos = glue::glue("{chr}_{pos}")) %>%
  select(chrpos, everything())

# Grab SNP genotypes for each sample
tob1524_snp <- select_snpchip_sample("TOB1524", chip_geno38, chip_id2tob_clean)
tob1532_snp <- select_snpchip_sample("TOB1532", chip_geno38, chip_id2tob_clean)
tob1533_snp <- select_snpchip_sample("TOB1533", chip_geno38, chip_id2tob_clean)

# Grab WGS genotypes for each sample
tob1524_wgs <-
...
# Calculate concordance stats between WGS and SNPchip for each sample
...
# Let's mix it up a bit and see
mixup_1524_vs_1533 <- concordance_stats(tob1524_wgs, tob1533_snp)
...
tab <-
  purrr::map_df(
    list(
      ...
      mixup_1524_vs_1533 = mixup_1524_vs_1533
    ), "summary",
    .id = "sample"
  )
