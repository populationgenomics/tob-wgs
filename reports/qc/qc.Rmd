...
Number of samples per flowcell-lane ID.
...
Removing lane labels with <2 samples.


Duplication rate against the lane and batch, grouped by batch:
...
Duplication rate against the lane and batch, grouped by lane:
...
```{r}
sex_data = all_data %>% 
  select(sample, chrX_ploidy, chrY_ploidy, sex_karyotype, gender, age) %>% 
  mutate(sex_mismatched = ifelse(sex_karyotype == "XX" & gender != 'F' | sex_karyotype == "XY" & gender != "M", T, F)) %>%
...
```{r}
sex_data %>% ggplot(aes(x = chrX_ploidy, y = chrY_ploidy, color = gender)) + 
  geom_point(size = 0.6) +
...
```{r}
sex_data %>% ggplot(aes(y = chrY_ploidy, x = age, color = gender)) + 
  geom_point(size = 0.6) +
  geom_text_repel(aes(label = label), color = "black", na.rm = TRUE, size = 2.3)
```
