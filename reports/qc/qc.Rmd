...
params:
  ...
  namespace: "test"
  dataset: "TOB-WGS"
  new_batches: "tob-wgs-1-PBMC,tob-wgs-2-PBMC,tob-wgs-1-bone-marrow,tob-wgs-2-bone-marrow"
...
meta = meta %>% mutate(
  ...
  batch = ifelse(
    resequencing_label != "-",
    str_c(batch, "-", resequencing_label),
    batch
  ),
  batch = ifelse(
    primary_study == "Pilot/bone marrow",
    str_c(batch, "-bone-marrow"),
    batch
  )
)

meta = meta %>% mutate(
  sample = ifelse(resequencing_label != "-"),
...
qc_plot_global <- p_data %>%
  ...
  scale_size_manual(values = c("Other batches" = 0.3, "New batches" = 1))
...
qc_plot_per_batch <- p_data %>%
  ggplot(aes(x = "", y = value, label = sample)) +
  geom_violin(fill = "transparent", colour = "grey80", alpha = 0.4) +
  geom_sina(aes(colour = Quality, group = metric), seed = 42, size = 0.8) +
  geom_hline(data = cutoffs, aes(yintercept = upper_lim), color="red") +
  geom_hline(data = cutoffs, aes(yintercept = lower_lim), color="blue") +
  facet_grid(
...
```{r}
sex_data = meta %>%
  mutate(sex_mismatched = ifelse(reported_sex == "F" & sex_karyotype != "XX" | reported_sex == "M"&& sex_karyotype != "XY", T, F)) %>%
  select(
    sex_mismatched,
    sample,
