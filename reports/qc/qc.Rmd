...
Table listing all samples {nrow(all_data)} samples in {n_batches} batches.
...
```{r}
sex_data = all_data %>% 
  select(sample, chrX_ploidy, chrY_ploidy, sex_karyotype, reported_sex, age) %>% 
  mutate(sex_mismatched = ifelse(sex_karyotype == "XX" & reported_sex != 'F' | sex_karyotype == "XY" & reported_sex != "M", T, F)) %>%
  mutate(label = ifelse(sex_mismatched, glue("{sample}, {sex_karyotype}, age: {age}"), NA_character_)) %>%
  arrange(sex_mismatched)
```

X ploidy vs Y ploidy. Samples with matched reported and inferred sex is labelled.

```{r}
sex_data %>% ggplot(aes(x = chrX_ploidy, y = chrY_ploidy, color = reported_sex)) + 
  geom_point(size = 0.6) +
  geom_text_repel(aes(label = label), color = "black", na.rm = TRUE, size = 2.3)
```

Y ploidy vs age. Samples with matched reported and inferred sex is labelled. chY degrades with age, so low chrY ploidies for older males are expected (though not high chrX ploidies).

```{r}
sex_data %>% ggplot(aes(y = chrY_ploidy, x = age, color = reported_sex)) + 
  geom_point(size = 0.6) +
  geom_text_repel(aes(label = label), color = "black", na.rm = TRUE, size = 2.3)
```
