...
```{r}
all_data %>% 
  count(batch, duplicates > 0.15, duplicates < 0.15,  duplicates > 0.12, duplicates > 0.12) %>%
  datatable()
```
...
p_data %>%
  ...
  geom_violin(fill = "transparent") +
  geom_sina(aes(colour = lane), seed = 42) +
  geom_text_repel(color = "black", na.rm = TRUE, size = 2.3) +
  scale_y_continuous(breaks = scales::breaks_pretty(8)) +
  theme(axis.ticks.x = element_blank(), axis.text.x = element_blank(),
        panel.grid.major.x = element_blank(),
...
lanes_with_more_than_2_samples <- all_data %>%
  group_by(lane) %>%
  tally %>% 
  filter(n > 2) %>% pull(lane) %>% unique()

p_data %>%
  filter(metric == 'duplicates') %>%
  filter(lane %in% lanes_with_more_than_2_samples) %>% 
  ggplot(aes(x = "", y = value, label = label, group = metric)) +
  geom_violin(fill = "transparent") +
...
Number of samples per lane ID:

```{r}
all_data %>% 
  count(lane) %>% 
  ggplot(aes(x = reorder(lane, -n), y = n)) + 
...
```{r}
my_data %>% 
  ggplot(aes(x = "", y = value, group=metric)) +
  geom_violin() +
  geom_sina()
```
