...
p_data <- all_data %>%
  ...
  mutate(
    ...
    label = NA_character_,
    label = if_else(is_outlier(value) & !high_quality,
                    glue("{sample} ({value_round}), {sample_filters}"), label),
    label = if_else(is_outlier(value) & high_quality,
                    glue("{sample} ({value_round})"), label),
    label = if_else(!is_outlier(value) & !high_quality,
                    glue("{sample}, {sample_filters}"), label))
...
```{r}
all_data %>% filter(batch == "2", duplicates < 0.15)
```
...
all_data %>%
  filter(
    (sex_karyotype == "XX" & gender != "F") |
      (sex_karyotype == "XY" & gender != "M")) %>%
  select(sample, sex_karyotype, reported = gender, batch,
         `chr20 mean dp`, contamination, sample_filters, high_quality)
```
