...
p_data <- qc_kccg %>%
  ...
  tidyr::pivot_longer(cols = -c("sample", "sample_filters", "high_quality"), names_to = "metric") %>%
  ...
  mutate(
    ...
    outlier = if_else(is_outlier(value), glue("{sample} ({value2})"), NA_character_)
  )
...
p_data %>% 
  ...
  geom_sina(aes(x = "", y = value)) +
  geom_text_repel(aes(x = "", y = value, label = outlier), color="black", na.rm = TRUE, size = 2.3) +
...
plot_width <- n_batches * 4
plot_height <- n_metrics * 2
```

Same, by groupped by batch:
...
p_data %>%
  ...
  ggforce::geom_sina(shape = 21, alpha = 0.40, fill = "red", seed = 42) +
  ggrepel::geom_text_repel(aes(x = "", y = value, label = outlier),
                           color = "black", na.rm = TRUE, size = 2.3, seed = 42) +
  geom_hline(data = cutoffs, aes(yintercept = upper_lim), color="red") +
  geom_hline(data = cutoffs, aes(yintercept = lower_lim), color="blue") +
