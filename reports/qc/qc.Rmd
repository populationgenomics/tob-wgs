...
p_data <- meta %>%
  ...
  mutate(
    ...
    label = if_else(
      is_outlier(value) | !high_quality, 
      glue("{sample} ({value_round})"), 
      NA_character_
    )
  )
...
Other samples failed QC (excluding sex mismatches, which is reported further below):
...
highlighted_batches_n = length(unique(p_data$highlighted_batch)) - 1
other_batches_n = length(unique(p_data$batch)) - highlighted_batches_n
...
qc_plot_per_batch <- p_data %>%
  ...
  labs(
    title = "QC metrics by batch",
    subtitle = glue(
      "Showing {highlighted_batches_n} batches, and other {other_batches_n} batches ",
      "groupped under 'Other samples'"
    )
  ) +
  xlab("")
...
sex_data = meta %>%
  mutate(
    ...
    bad_sex = is_new_batch & (sex_mismatched | sex_aneuploidy | ambiguous_sex)
  ) %>%
  select(
    sex_mismatched,
    sex_aneuploidy,
