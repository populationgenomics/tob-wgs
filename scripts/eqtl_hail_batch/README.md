...
Base analysis (Round 1)
    ...
    * Step 1: Fit a linear model to the expression data in order to account for experimental covariates (sex, age, PCs 1-4, Peer Factors 1 & 2). Take the residuals, which are then used as the adjusted expression values.
    Step 2: Test the association of all SNPs to the adjusted expression values
...
Conditional analysis (Rounds 2-5)
    ...
    Step 1: Fit a model to the adjusted expression data to account for the genotype of the most significant SNP. Take the residuals, which are the SNP + covariate adjusted expression values
    Step 2: Test the association of significant SNPs from the previous round against SNP & covariate adjusted expression data
