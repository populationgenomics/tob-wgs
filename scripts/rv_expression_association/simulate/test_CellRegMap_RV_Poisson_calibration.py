...
def get_crm_pvs(pheno, covs, genotypes, E=None, Poisson=False):
    """
    CellRegMap-RV tests (Poisson version)
    * score test (variance)
    * burden test (max, sum, comphet)
    * omnibus (Cauchy) test
    """
    pv0 = run_gene_set_association(y=pheno, G=genotypes, W=covs, E=E, Poisson=Poisson)[0]
    pv1 = run_burden_association_glmm(y=pheno, G=genotypes, W=covs, E=E, mask="mask.max")[0]
    pv2 = run_burden_association_glmm(y=pheno, G=genotypes, W=covs, E=E, mask="mask.sum")[0]
    pv3 = run_burden_association_glmm(
...
for i in range(n_reps):
    ...
    pv_scenario1_mt[i, 9:17] = get_crm_pvs(pheno_pois, covs, alt_genotypes, E, Poisson=True)
...
pv_scenario1_filename = AnyPath(
    output_path('simulations/CRM_Poisson/1000samples_10causal_singletons/shuffled/10tested_samebeta.csv')
)
...
for i in range(n_reps):
    ...
    pv_scenario2_mt[i, 9:17] = get_crm_pvs(pheno_pois, covs, alt_genotypes, E, Poisson=True)
...
pv_scenario2_filename = AnyPath(output_path('simulations/CRM_Poisson/1000samples_10causal_singletons/shuffled/50tested_samebeta.csv'))
...
for i in range(n_reps):
    seed(i)
    select_singletons_20 = set(sample(singletons, 20))
    genotypes = geno_subset[select_singletons_20]  # subset genotypes
    beta = zeros((genotypes.shape[1], 1))          # create betas as 0s
    beta[0:10] = 1                                 # only 10 non-0 betas
    # get other singletons to test (to assess calibration)
    singletons_left = singletons - select_singletons_20
    seed(i)
    alt_singletons_20 = sample(singletons_left, 20)
    ...
    pv_scenario2a_mt[i, 9:17] = get_crm_pvs(pheno_pois, covs, alt_genotypes, E, Poisson=True)
...
pv_scenario2a_filename = AnyPath(output_path('simulations/CRM_Poisson/1000samples_10causal_singletons/shuffled/20tested_samebeta.csv'))
...
for i in range(n_reps):
    ...
    pv_scenario3_mt[i, 9:17] = get_crm_pvs(pheno_pois, covs, alt_genotypes, E, Poisson=True)
...
pv_scenario3_filename = AnyPath(output_path('simulations/CRM_Poisson/1000samples_10causal_singletons/shuffled/10tested_2negativebeta.csv'))
...
for i in range(n_reps):
    ...
    pv_scenario3a_mt[i, 9:17] = get_crm_pvs(pheno_pois, covs, alt_genotypes, E, Poisson=True)
...
pv_scenario3a_filename = AnyPath(output_path('simulations/CRM_Poisson/1000samples_10causal_singletons/shuffled/10tested_5negativebeta.csv'))
...
for i in range(n_reps):
    ...
    pv_scenario4_mt[i, 9:17] = get_crm_pvs(pheno_pois, covs, alt_genotypes, E, Poisson=True)
...
pv_scenario4_filename = AnyPath(output_path('simulations/CRM_Poisson/1000samples_10causal_singletons/shuffled/10tested_varyingbeta.csv'))
